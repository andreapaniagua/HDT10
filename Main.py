...
while opcion != "6":
    ...
    if opcion == "1":
        ...
        db.run("MATCH (p:Paciente),(d:Doctor),(m:Medicina) WHERE p.nombre = '"+nomPac+"' AND d.nombre = '"+nomDoc"' AND m.nombre = '"+medicina"' CREATE (p)-[r:VISITS{date: '"+ fecha +"'}]->(d) -> [r:PRESCRIBES] -> (m) -> [r: TAKES] -> (p) ")
        ...
        if opc == "1":
            ...
            db.run("MATCH (doc1:Doctor),(doc2:Doctor) WHERE doc1.nombre = '"+d1+"' AND doc2.nombre = '"+d2"'CREATE (doc1)<-[r:KNOWS]->(doc2)")
            
        else if opc == "2":
            
            p1 = input("Ingrese el nombre del primer paciente")
            p2 = input("Ingrese el nombre del segundo paciente")
            db.run("MATCH (pac1:Paciente),(pac2:Paciente) WHERE pac1.nombre = '"+p1+"' AND pac2.nombre = '"+p2"'CREATE (pac1)<-[r:KNOWS]->(pac2)")
